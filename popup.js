...
function handleKeyPress(e) {
  ...
  if (e.key === "ArrowDown") {
    ...
    if (
      ...
      currentSelectionIndex < filteredBookmarks.length - 1
    ) {
      currentSelectionIndex++;
      displayResults();
    }
...
function checkExactMatch() {
  const exactMatch = bookmarks.find(
    (b) => b.key.toLowerCase() === inputText.toLowerCase(),
  );
  if (exactMatch) {
...
function displayResults() {
  ...
  const html = filteredBookmarks
    .map((bookmark, index) => {
      ...
      const className =
        index === currentSelectionIndex ? "result-item active" : "result-item";
      return `
      <div class="${className}">
...
function scrollToSelectedItem() {
  if (
    ...
    currentSelectionIndex < filteredBookmarks.length
  ) {
